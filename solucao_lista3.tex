...
\solucao{
	...
	\textbf{Fundamentação Teórica:}
	\begin{enumerate}
		\item \textbf{Definição de Subvariedade Mergulhada (Lee, Cap. 8):} Se $S$ é uma subvariedade mergulhada de dimensão 1, então para cada $p \in S$, deve existir uma parametrização suave $\gamma : (-\epsilon, \epsilon) \to S$ tal que $\gamma(0)=p$ e $\gamma'(0) \neq (0,0)$.
	...
	Como a curva $\gamma$ deve estar contida em $S$, a condição $x(t) \geq 0$ deve ser satisfeita para todo $t \in (-\epsilon, \epsilon)$. Dado que $x(0)=0$, o ponto $t=0$ é um mínimo local da função $x(t)$. Pela teoria do cálculo, isso implica que:
	\begin{equation*}
		x'(0) = 0.
	\end{equation*}
...
\solucao{
	...
	\begin{itemize}
		...
		\item \textbf{Tese:} Provar a igualdade de subespaços $T_p S = \ker(d\Phi_p)$ para todo $p \in S \cap U$.
	\end{itemize}
	
	\textbf{Fundamentação Teórica:}
	\begin{enumerate}
		\item \textbf{Definição de $T_p S$ (Lee, Cap. 8):} Para uma subvariedade mergulhada $S$, o espaço tangente $T_p S$ é o subespaço de $T_p M$ consistindo em todos os vetores $\gamma'(0)$ onde $\gamma : (-\epsilon, \epsilon) \to S$ é uma curva suave com $\gamma(0) = p$.
		\item \textbf{Aplicação que Define Localmente $S$ (Lee, Cap. 7):} Dizer que $\Phi$ define localmente $S$ significa que $S \cap U = \Phi^{-1}(c)$ para algum valor regular $c \in N$. Portanto, $d\Phi_p : T_p M \to T_c N$ é sobrejetiva.
		\item \textbf{Regra da Cadeia (Lee, Cap. 3):} Se $\gamma$ é uma curva em $S$, então $\Phi \circ \gamma$ é uma curva em $N$. Sua derivada é dada por $d\Phi_p(\gamma'(0))$.
	\end{enumerate}
	
	\textbf{Desenvolvimento Formal:}
	
	Para provar a igualdade de subespaços $T_p S = \ker(d\Phi_p)$, provaremos a inclusão em ambas as direções.
	
	\textit{1. Inclusão $T_p S \subseteq \ker(d\Phi_p)$:}
	Seja $v \in T_p S$. Por definição, existe uma curva suave $\gamma : (-\epsilon, \epsilon) \to S$ tal que $\gamma(0) = p$ e $\gamma'(0) = v$. Como a imagem de $\gamma$ está contida em $S$, e $S$ é o conjunto de nível $\Phi^{-1}(c)$, temos que:
	\begin{equation*}
		\Phi(\gamma(t)) = c \quad \text{para todo } t \in (-\epsilon, \epsilon).
	\end{equation*}
	Diferenciando ambos os lados em relação a $t$ no ponto $t=0$, e utilizando a Regra da Cadeia:
	\begin{equation*}
		\frac{d}{dt} (\Phi \circ \gamma)(t) \Big|_{t=0} = d\Phi_p(\gamma'(0)) = d\Phi_p(v).
	\end{equation*}
	Como a função constante $\Phi(\gamma(t)) = c$ possui derivada nula, segue que $d\Phi_p(v) = 0$. Portanto, $v \in \ker(d\Phi_p)$.
	
	\textit{2. Inclusão $\ker(d\Phi_p) \subseteq T_p S$:}
	Utilizaremos a comparação de dimensões. 
	Pelo Teorema do Valor Regular, como $\Phi$ define localmente $S$, a dimensão de $S$ é $\dim S = \dim M - \dim N = n - \dim N$. Assim, $\dim T_p S = n - \dim N$.
	Por outro lado, a aplicação $d\Phi_p : T_p M \to T_c N$ é sobrejetiva (posto máximo). Pelo Teorema do Núcleo e da Imagem (Álgebra Linear):
	\begin{equation*}
		\dim(\ker d\Phi_p) = \dim T_p M - \text{posto}(d\Phi_p) = n - \dim N.
	\end{equation*}
	Como $T_p S$ é um subespaço de $\ker(d\Phi_p)$ e ambos possuem a mesma dimensão finita $n - \dim N$, eles devem ser obrigatoriamente o mesmo subespaço.
	
	Conclui-se que $T_p S = \ker(d\Phi_p)$.
}