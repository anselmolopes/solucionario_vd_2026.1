	...
	\solucaoalt{
		...
		\begin{figure}[htbp]
			...
			\begin{tikzpicture}[node distance=2.5cm, auto, >=Stealth]
				...
				\draw[->, thick] (P1) -- node[below] {$\Theta_{\alpha\beta}(p,v) = (p, \tau_{\alpha\beta}(p)v)$} (P2);
			\end{tikzpicture}
	...
	\solucao{
		...
		\begin{figure}[htbp]
			...
			\begin{tikzpicture}[node distance=3cm, auto, >=Stealth]
				...
				% Setas
				\draw[->, thick] (U) -- node {$\sigma|_U$} (E);
				\draw[->, thick] (E) -- node {$\Phi$} (R);
				\draw[->, thick] (U) -- node[swap, yshift=-4pt] {$\hat{\sigma}(q) = (q, 0)$} (R);
			\end{tikzpicture}
...
\questao{
	Seja $M^{n}$ uma $n$-variedade suave. Então a estrutura suave já construída no fibrado tangente $TM$ é a única com respeito a qual $\pi : TM \to M^n$ é um fibrado vetorial suave de posto $n$ sobre $M^n$ e cujos campos vetoriais coordenados são seções locais suaves.
}

\solucao{
	\textbf{Análise do Enunciado}
	\begin{itemize}
		\item \textbf{Hipóteses:} $\pi: TM \to M^n$ é dotada de uma estrutura de fibrado vetorial suave de posto $n$. Os campos vetoriais coordenados $\left\{ \frac{\partial}{\partial x^1}, \dots, \frac{\partial}{\partial x^n} \right\}$, associados a qualquer carta de $M^n$, são exigidos como seções locais suaves.
		\item \textbf{Tese:} Provar que esta exigência força a estrutura suave de $TM$ a coincidir com a estrutura canônica.
	\end{itemize}
...
% =============================================================================
% INSTRUMENTO ALGÉBRICO: DEFINIÇÃO DOS QUATÉRNIOS
% =============================================================================
...
\questao{
	...
	$$X_{1}=-x^{2}\frac{\partial}{\partial x^{1}}+x^{1}\frac{\partial}{\partial x^{2}}+x^{4}\frac{\partial}{\partial x^{3}}-x^{3}\frac{\partial}{\partial x^{4}}$$
...
\solucao{
	...
	\textit{Condição Necessária ($\implies$):} Assumimos que $\omega$ é uma seção suave. Seja $X = X^j \frac{\partial}{\partial x^j}$ um campo de vetores suave em $U$. A função $\omega(X): U \to \mathbb{R}$ é dada pela contração:
	\begin{equation*}
		\omega(X)(p) = \langle \omega_p, X_p \rangle = \langle \omega_i dx^i|_p, X^j \tfrac{\partial}{\partial x^j} \Big|_p \rangle = \omega_i(p) X^j(p) \delta^i_j = \omega_i(p) X^i(p).
	\end{equation*}
	Visto que $\omega_i$ e $X^i$ são funções suaves em $U$, a soma de seus produtos $\sum \omega_i X^i$ é também uma função suave.
	
	\textit{Condição Suficiente ($\impliedby$):} Assumimos que a função $\omega(X)$ é suave para todo campo de vetores suave $X \in \mathfrak{X}(U)$.
	
	Para provar que $\omega$ é suave, utilizaremos a estratégia de "testar" a seção com os campos de vetores coordenados. Para cada $j \in \{1, \dots, n\}$, definimos o campo $X_{(j)} = \frac{\partial}{\partial x^j}$. Como estes são campos coordenados, são necessariamente suaves. Pela nossa hipótese, a função $\omega(X_{(j)})$ deve ser suave:
	\begin{equation*}
		\omega(X_{(j)})(p) = \langle \omega_i dx^i|_p, \tfrac{\partial}{\partial x^j} \Big|_p \rangle = \omega_i(p) \delta^i_j = \omega_j(p).
	\end{equation*}
	Isso demonstra que cada função componente $\omega_j$ é suave. Pelo resultado provado na \textbf{Parte (a)}, a suavidade de todas as componentes $\omega_j$ implica a suavidade da seção $\omega$.
}
...
\solucao{
	...
	\begin{enumerate}
		\item \textbf{Diferencial de uma função:} Para $f \in C^\infty(M)$, a diferencial $df$ é a aplicação que associa a cada ponto $p \in M$ um covetor $df_p \in T_p^* M$. Localmente, $df = \sum \frac{\partial f}{\partial x^i} dx^i$.
		\item \textbf{Pullback de diferenciais:} Seja $F : M \to N$ uma aplicação suave. O pullback $F^* df$ é definido tal que, para qualquer vetor $X \in T_p M$, temos $\langle (F^* df)_p, X \rangle = \langle df_{F(p)}, F_{*,p} X \rangle$. Em coordenadas, se $df = \sum a_j dy^j$, então $F^* df = \sum (a_j \circ F) d(F^j)$.
		\item \textbf{Aplicação $F$:} A inversa da projeção estereográfica é dada por:
		\begin{equation*}
	...
	Seja $D = u^2 + v^2 + 1$. As componentes de $F(u,v) = (F_1, F_2, F_3)$ são $F_1 = \frac{2u}{D}$, $F_2 = \frac{2v}{D}$ e $F_3 = \frac{u^2+v^2-1}{D}$.
	
	\textbf{Cálculo 1: A diferencial da composição $d(f \circ F)$}
	
	Substituímos as componentes de $F$ na função $f$:
	\begin{align*}
		(f \circ F)(u,v) &= (F_1)^2 + (F_2)^2 + (F_3)^2 \\
		&= \left( \frac{2u}{D} \right)^2 + \left( \frac{2v}{D} \right)^2 + \left( \frac{u^2+v^2-1}{D} \right)^2 \\
		&= \frac{4u^2 + 4v^2 + (u^2+v^2-1)^2}{D^2}.
	\end{align*}
	Expandindo o termo quadrático $(u^2+v^2-1)^2 = (u^2+v^2)^2 - 2(u^2+v^2) + 1$:
	\begin{align*}
		(f \circ F)(u,v) &= \frac{4(u^2+v^2) + (u^2+v^2)^2 - 2(u^2+v^2) + 1}{D^2} \\
		&= \frac{(u^2+v^2)^2 + 2(u^2+v^2) + 1}{D^2} \\
		&= \frac{(u^2+v^2+1)^2}{D^2} = \frac{D^2}{D^2} = 1.
	\end{align*}
	Como a composição $f \circ F$ é a função constante $1$ em todo o domínio $\mathbb{R}^2$, sua diferencial é nula:
	\begin{equation*}
		\boxed{d(f \circ F) = 0}.
	\end{equation*}
	
	\textbf{Cálculo 2: O pullback $F^* df$}
	
	Calculamos a diferencial de $f$ em $\mathbb{R}^3$:
	\begin{equation*}
		df = 2x\,dx + 2y\,dy + 2z\,dz.
	\end{equation*}
	Aplicamos a definição de pullback $F^* df = 2 F_1 dF_1 + 2 F_2 dF_2 + 2 F_3 dF_3$. Calculamos as diferenciais de cada componente de $F$:
	\begin{align*}
		dF_1 &= \frac{\partial F_1}{\partial u} du + \frac{\partial F_1}{\partial v} dv = \frac{2(D) - 2u(2u)}{D^2} du + \frac{2u(-2v)}{D^2} dv = \frac{2(1+v^2-u^2)}{D^2} du - \frac{4uv}{D^2} dv, \\
		dF_2 &= \frac{\partial F_2}{\partial u} du + \frac{\partial F_2}{\partial v} dv = \frac{2v(-2u)}{D^2} du + \frac{2(D) - 2v(2v)}{D^2} dv = -\frac{4uv}{D^2} du + \frac{2(1+u^2-v^2)}{D^2} dv, \\
		dF_3 &= \frac{\partial F_3}{\partial u} du + \frac{\partial F_3}{\partial v} dv = \frac{2u(D) - (u^2+v^2-1)(2u)}{D^2} du + \frac{2v(D) - (u^2+v^2-1)(2v)}{D^2} dv \\
		&= \frac{4u}{D^2} du + \frac{4v}{D^2} dv.
	\end{align*}
	
	Somamos os termos para obter a componente de $du$:
	\begin{align*}
		(F^* df)_u &= 2 F_1 \frac{\partial F_1}{\partial u} + 2 F_2 \frac{\partial F_2}{\partial u} + 2 F_3 \frac{\partial F_3}{\partial u} \\
		&= 2\left( \frac{2u}{D} \right) \left( \frac{2(1+v^2-u^2)}{D^2} \right) + 2\left( \frac{2v}{D} \right) \left( \frac{-4uv}{D^2} \right) + 2\left( \frac{u^2+v^2-1}{D} \right) \left( \frac{4u}{D^2} \right) \\
		&= \frac{8u + 8uv^2 - 8u^3 - 16uv^2 + 8u^3 + 8uv^2 - 8u}{D^3} = 0.
	\end{align*}
	
	Calculamos a componente de $dv$:
	\begin{align*}
		(F^* df)_v &= 2 F_1 \frac{\partial F_1}{\partial v} + 2 F_2 \frac{\partial F_2}{\partial v} + 2 F_3 \frac{\partial F_3}{\partial v} \\
		&= 2\left( \frac{2u}{D} \right) \left( \frac{-4uv}{D^2} \right) + 2\left( \frac{2v}{D} \right) \left( \frac{2(1+u^2-v^2)}{D^2} \right) + 2\left( \frac{u^2+v^2-1}{D} \right) \left( \frac{4v}{D^2} \right) \\
		&= \frac{-16u^2v + 8v + 8u^2v - 8v^3 + 8u^2v + 8v^3 - 8v}{D^3} = 0.
	\end{align*}
	...
	\textbf{Conclusão}
	Ambos os cálculos resultam em zero, verificando a identidade.
}

\solucaoalt{
	\textbf{Demonstração via Operadores de Derivação}
	
	Seguindo a lógica de que vetores tangentes são derivações, podemos provar a igualdade sem a expansão algébrica do pullback.
	
	\textbf{1. Análise da função composta:}
	Como demonstrado na solução principal, a composição $(f \circ F)(u,v) = 1$. Sendo esta uma função constante, sua diferencial é nula:
	\begin{equation*}
		d(f \circ F) = 0.
	\end{equation*}
	
	\textbf{2. Análise do pullback via derivações:}
	Seja $X \in T_p \mathbb{R}^2$ um vetor tangente qualquer. Pela definição de pullback e pela propriedade do diferencial como derivação:
	\begin{align*}
		\langle (F^* df)_p, X \rangle &= \langle df_{F(p)}, F_{*,p} X \rangle \\
		&= (F_{*,p} X)(f) \\
		&= X(f \circ F).
	\end{align*}
	Como $f \circ F$ é a função constante $1$, e qualquer derivação $X$ aplicada a uma constante resulta em zero, temos:
	\begin{equation*}
		\langle (F^* df)_p, X \rangle = X(1) = 0.
	\end{equation*}
	Visto que a contração de $F^* df$ com qualquer vetor tangente $X$ é nula, concluímos que:
	\begin{equation*}
		F^* df = 0.
	\end{equation*}
	Ambos os resultados coincidem, validando a identidade.
}