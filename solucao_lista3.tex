...
\solucao{
	...
	\textbf{Fundamentação Teórica:}
	\begin{enumerate}
		\item \textbf{Definição de $\iota$-relacionamento (Lee, Cap. 4):} Um campo $X \in \mathfrak{X}(S)$ é $\iota$-relacion도 a $Y \in \mathfrak{X}(M)$ se, para todo $p \in S$, temos $\iota_{*,p} X(p) = Y(p)$.
