...
\solucao{
	...
	Consideremos as aplicações \textbf{contínuas} $\alpha, \beta: \mathbb{S}^n \times \mathbb{S}^n \to \mathbb{R}^{n+1}$, dadas por dada por $\alpha(x,y) = x - y$ e $\beta(x,y) = x + y$. Como $\{0\}$ é um conjunto fechado em $\mathbb{R}^{n+1}$, suas pré-imagens $\alpha^{-1}(\{0\})$ e $\beta^{-1}(\{0\})$ são fechadas em $\mathbb{S}^n \times \mathbb{S}^n$. Assim, a relação $R=\alpha^{-1}(\{0\}) \cup \beta^{-1}(\{0\})$ é a união finita de conjuntos fechados, e portanto fechado em $\mathbb{S}^n \times \mathbb{S}^n$. Logo, $\mathbb{RP}^n$ herda a propriedade de ser Hausdorff.
